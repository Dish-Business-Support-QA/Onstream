...
from Safari_Thread import version, mc, ChannelCount, device, GetService
...
class TestHomeScreen:
    def test_images_displayed(self):
        try:
            ...
            live = self.driver.find_elements(By.XPATH,
                                             '//div[@class="_1acuZqkpaJBNYrvoPzBNq_ _1Ec0IteN1F_Ae9opzh37wr"]')
            for image in live:
    ...
    def test_buttons_displayed(self):
        try:
            ...
            live = self.driver.find_elements(By.XPATH,
                                             '//div[@class="_1iKpTFW64nBCEODaArwlyd _1encUiSOWTmH2vOVl5BZqy"]')
            for button in live:
...
class TestLiveTV:
    def test_images_displayed(self):
        try:
            ...
            WebDriverWait(self.driver, 10).until(ec.visibility_of_element_located((By.XPATH, '//img[@alt="%s"]' % ChannelCount.first_channel)))  # wait for the guide to populate
            channel = self.driver.find_elements(By.XPATH, '//div[@class="kRqPgjOjFTl1XZ21adOmo"]')  # click on first channel
            for i in range(100):
                try:
                    channel[i].click()
            ...
            WebDriverWait(self.driver, 30).until(ec.presence_of_element_located((By.XPATH, '//img[@id="bmpui-id-32"]')))
    ...
    def test_control_bar_functions(self):
        try:
            ...
            WebDriverWait(self.driver, 30).until(ec.visibility_of_element_located((By.XPATH, '//button[@class="bmpui-ui-volumetogglebutton bmpui-muted"]'))).is_displayed()  # Mute button on
            self.driver.find_element(By.XPATH, '//button[@class="bmpui-ui-volumetogglebutton bmpui-muted"]').click()  # Mute button turn off
            WebDriverWait(self.driver, 30).until(ec.visibility_of_element_located((By.XPATH, '//button[@class="bmpui-ui-volumetogglebutton bmpui-unmuted"]'))).is_displayed()  # Mute button off
            # volume slider bar
...
class TestLegalSettingsScreen:
    def test_images_displayed(self):
        try:
            WebDriverWait(self.driver, 30).until(ec.presence_of_element_located((By.XPATH, '//button[@class="_2YXx31Mkp4UfixOG740yi7 schema_accent_background"]'))).click()
            settings = WebDriverWait(self.driver, 30).until(ec.visibility_of_element_located((By.XPATH, '(//a[@role="button"])')))  # go to the play button
