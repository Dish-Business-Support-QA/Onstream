...
def pytest_run():
    ...
    while int(len(channels)) < int(cc.get_number()):
        mc.increment()
        mc.save_value()
        subprocess.run(['pytest', '--pytest-influxdb', '--influxdb_project=iPhone_Safari', '--influxdb_run_id=' + str(mc.get_value()), os.path.join(test_path, 'OnStream_iPad_Safari.py'), '-sv'])
