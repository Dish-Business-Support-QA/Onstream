...
class ChannelCount(object):
    ...
    for i in range(len(
            logos)):  # A for loop that collects certain information from the OnStream guide which will be validated against the JSON information
...
for gu in ChannelCount.all_guide_uid:  # A for loop which compares the list of JSON data with the list of Guide Data in OnStream (SUID)
    for logo in all_ld:
        if str(logo['suid']) in str(gu):
            print("found")

"""for gi in ChannelCount.all_guide_images:
    for logo in all_ld:
        if str(logo['logo_url']) in str(gi):
            print("found")"""
